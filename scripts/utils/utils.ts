...
function extractExerciseNumber(dir: string) {
    ...
    if (!number) {
        throw new Error(
            `Exercise directory ${dir} does not match regex ${regex}`,
        )
    }
    return Number(number)
}
                ...
                return Promise.all(
                    dirs.map(async function getAppFromPath(dir) {
                        ...
                        if (dir.includes(".extra-")) {
                            const extraCreditNumber =
                                extractExtraCreditNumber(dir)
                            return {
                                type: "extra-credit-exercise",
                                exerciseNumber,
                                extraCreditNumber,
                                ...
                                title,
                                portNumber:
                                    4050 + exerciseNumber + extraCreditNumber,
                            }
                        } else {
                            return {
                                type: "exercise",
                    ...
                    dirs.map(async function getAppFromPath(dir) {
                        ...
                        if (dir.includes(".extra-")) {
                            ...
                            return {
                                ...
                                title,
                                portNumber:
                                    5050 + exerciseNumber + extraCreditNumber,
                            }
                        } else {
...
export async function guessNextApp(app: App) {
    ...
    if (app.type === "exercise" || app.type === "extra-credit-exercise") {
        const finalVersion = apps.find(
            (a) =>
                a.relativePath ===
                app.relativePath.replace("exercise", "final"),
        )
    ...
    const indexOfApp = apps.findIndex(
        (a) => a.relativePath === app.relativePath,
    )
